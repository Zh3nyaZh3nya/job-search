...
const activeResume = ref<boolean>(props.card.active)
const category = ref<Category>(props.card.category)
...
const salaryFrom = ref<number | null>(props.card.from_salary || null)
...
const infoWorkExperience = ref<IResume['info_resume']['work_experience']>(props.card.info_resume.work_experience)
const educationExp = ref<IResume['info_resume']['education']>(props.card.info_resume.education)
...
async function editResume() {
  ...
  await store.editResume(bodyData)
}
</script>
